...
local soundList = {
  [1] =  { name = "none",        path = "" },
  [2] =  { name = "Raid Warn",   path = "Sound\\interface\\RaidWarning.wav" },
  [3] =  { name = "Moo",         path = "Sound\\Spells\\PolyMorphCow.wav" },
  [4] =  { name = "Moo2",        path = "Sound\\Creature\\Cow\\CowDeath.wav" },
  [5] =  { name = "Ta-Da",       path = "Sound\\Spells\\PVPFlagTaken.wav" },
  [6] =  { name = "Tadpole",     path = "Sound\\Creature\\BabyMurloc\\BabyMurlocA.wav" },
  [7] =  { name = "Yahh",        path = "Sound\\Character\\Human\\MaleNPC\\HumanMalePirateHeavy\\HumanMalePirateHeavyAggro.wav" },
  [8] =  { name = "Murmur",      path = "Sound\\Creature\\Murmur\\MurmurSpawn.wav" },
  [9] =  { name = "Turkey",      path = "Sound\\Creature\\turkey\\TurkeyAttackA.wav" },
  [10] = { name = "Klunk",       path = "Sound\\Doodad\\ArcaneCrystalOpen.wav" },
  [11] = { name = "Gong",        path = "Sound\\Doodad\\G_GongTroll01.wav" },
  [12] = { name = "Gong2",       path = "Sound\\Doodad\\BellTollHorde.wav" },
  [13] = { name = "Drum",        path = "Sound\\Doodad\\BellTollTribal.wav" },
  [14] = { name = "Ding Ding",   path = "Sound\\Doodad\\BoatDockedWarning.wav" },
  [15] = { name = "Boom",        path = "Sound\\Doodad\\Cannon01_BlastA.wav" },
  [16] = { name = "Klank",       path = "Sound\\Doodad\\DireMaulCrystalGeneratorOpen.wav" },
  [17] = { name = "Thunk",       path = "Sound\\Item\\Weapons\\Shields\\MetalShieldBlock2.wav" },
...
function MultiTool:rpcCommandSenderWrapper( rpcArgs )
  self:debugMsg("rpcCommandSenderWrapper()", "debug");
  self:debugMsg("  prefix: "..tostring( rpcArgs.prefix ), "result");
  self:debugMsg("  text: "..tostring( rpcArgs.text ), "result");
  self:debugMsg("  distribution: "..tostring( rpcArgs.distribution ), "result");
  self:debugMsg("  target: "..tostring( rpcArgs.target ), "result");
  self:debugMsg("  prio: "..tostring( rpcArgs.prio ), "result");
  self:debugMsg("  noComm: "..tostring( rpcArgs.noComm ), "result");
...
function MultiTool:autoRepair()
  if (self.db.profile.repairFlag) then
    ...
    if (CanMerchantRepair()) then
      ...
      if (needRepair) then
        -- Do the repair
        RepairAllItems(self.db.profile.repairGuildFlag)
        ...
        if (self.db.profile.repairWarnFlag) then
          ...
          local msg = {
            ...
            guild_funds = tostring(self.db.profile.repairGuildFlag)
          }
          ...
          if (self.db.profile.repairWhisperFlag) then
            self:debugMsg("  repairWhisperFlag is true: Changing distribution to WHISPER", "debug")
...
function MultiTool:OnCommReceived(prefix, message, distribution, target,foo)
  ...
  if (self.db.profile.debugFlag) then
    self:debugMsg("OnCommReceived():: ENGAGED", "debug")
    self:debugMsg("  prefix:"..tostring(prefix), "blather")
    self:debugMsg("  message:"..tostring(message), "blather")
    self:debugMsg("  distribution:"..tostring(distribution), "blather")
    self:debugMsg("  target:"..tostring(target), "blather")
    self:debugMsg("  player:"..UnitName("player"), "blather")
    self:debugMsg("  Deserialize success:"..tostring(success), "blather")
    self:debugMsg("  Message Content:"..tostring(o), "blather")
  ...
  if (success == false) then
    ...
    if (o["op"] == "foo") then
      if (self.db.profile.actionFlag) then
        ...
        if ("someCondition" == "some value") then
          self:debugMsg("debug_text_here", "blather")
          return -- do nothing
        else
          -- do the action
          self:debugMsg("debug_text_here", "blather")
    ...
    if (o["op"] == "QUEST_GOSSIP") then
      if (self.db.profile.gossipFlag) then
        ...
        if ("someCondition" == "some bad value") then
          -- conditions mean we DON'T do anything
          self:debugMsg("Global QUEST_GOSSIP global condition failed - not doing anything", "blather")

          return -- do nothing
        else
          -- do the action
          self:debugMsg("QUEST_GOSSIP global conditions succeded - parsing subcommands", "blather")
...
function MultiTool:rpcCommandSender(prefix, text, distribution, target, prio, noComm)
  --debug
  if (self.db.profile.debugFlag) then
    self:debugMsg("rpcCommandSender()", "debug")
    self:debugMsg("  prefix: "..tostring(prefix), "blather")
    self:debugMsg("  text: "..tostring(text), "blather")
    self:debugMsg("  distribution: "..tostring(distribution), "blather")
    self:debugMsg("  target: "..tostring(target), "blather")
    self:debugMsg("  prio: "..tostring(prio), "blather")
    self:debugMsg("  noComm flag: "..tostring(noComm), "blather")
  end

  -- New throttling - DO NOT send msg unless noComm is nil
  if (noComm) then
    self:debugMsg("noComm flag IS set, NOT sending command", "blather")
    -- Teh Gogglez - They do NOTHING!
    return
  else
    self:debugMsg("noComm flag NOT set, sending command", "blather")
    if (GetNumPartyMembers() == 0 and (distribution == "RAID" or distribution == "PARTY")) then
...
function MultiTool:getAvailableQuestsTable()
  ...
  for i = 1, #options, 3 do
    returnTable[i] = options[i];
    self:debugMsg("    adding "..tostring(options[i]), "blather");
  end 
